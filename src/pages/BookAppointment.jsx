...
export default function BookAppointment() {
  ...
  const handleDoctorSelect = async (doc) => {
    ...
    try {
      ...
      const activeSchedules = response.data.filter(sch => sch.availableDate >= todayStr);
  ...
  const handleConfirmBooking = async () => {
    ...
    try {
      ...
      setMessage({ text: 'Your Appointment has been successfully booked! 🎉🎟️', type: 'success' });
      setTimeout(() => {
        setStep(1);
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto space-y-8">
      <div className="text-center">
        <h2 className="text-3xl font-extrabold text-slate-800">Book An Appointment</h2>
        <p className="text-sm text-gray-500 mt-2">Get channelled with our specialized doctors in 3 simple steps.</p>
      </div>

      <div className="flex items-center justify-center space-x-4 border-b border-gray-100 pb-5">
        <div className={`px-4 py-2 rounded-xl text-xs font-bold transition ${step === 1 ? 'bg-blue-600 text-white shadow-md' : 'bg-gray-100 text-gray-500'}`}>
          1. Choose Doctor
        </div>
        <div className="text-gray-300">➔</div>
        <div className={`px-4 py-2 rounded-xl text-xs font-bold transition ${step === 2 ? 'bg-blue-600 text-white shadow-md' : 'bg-gray-100 text-gray-500'}`}>
          2. Select Session
        </div>
        <div className="text-gray-300">➔</div>
        <div className={`px-4 py-2 rounded-xl text-xs font-bold transition ${step === 3 ? 'bg-blue-600 text-white shadow-md' : 'bg-gray-100 text-gray-500'}`}>
          3. Confirm Booking
        </div>
      </div>

      {message.text && (
        <div className={`p-4 rounded-xl text-sm font-semibold border text-center transition-all ${
      ...
      {step === 1 && (
        <div className="space-y-6">
          <div className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm max-w-md mx-auto">
            <label className="block text-sm font-semibold text-gray-700 mb-2">Filter by Specialization</label>
            <select
          ...
          {selectedSpecialization && (
            <div className="space-y-4">
              <h3 className="text-lg font-bold text-slate-700">Available Doctors ({doctors.length})</h3>
              {doctors.length === 0 ? (
                <p className="text-sm text-gray-400 italic">No doctors available under this specialization currently.</p>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {doctors.map((doc) => (
                    <div key={doc.id} className="bg-white p-5 rounded-2xl border border-gray-100 shadow-sm flex justify-between items-center hover:border-blue-500 transition-all group">
                      <div>
                        <h4 className="font-bold text-slate-800 text-base group-hover:text-blue-600 transition">{doc.user?.name}</h4>
                        <p className="text-xs text-gray-400 mt-0.5">✉️ {doc.user?.email}</p>
      ...
      {step === 2 && (
        <div className="space-y-5">
          <div className="flex justify-between items-center">
            <button onClick={() => setStep(1)} className="text-sm text-blue-600 hover:underline cursor-pointer font-medium">
              ⬅ Back to Doctors
            </button>
            <h3 className="text-lg font-bold text-slate-700">Available Sessions for {selectedDoctor?.user?.name}</h3>
          </div>
          ...
          {loading ? (
            ...
            <div className="bg-white p-8 text-center rounded-2xl border border-gray-100 text-gray-400 italic shadow-sm">
              This doctor has no active availability sessions scheduled at the moment.
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {schedules.map((sch) => (
                ...
                >
                  <div className="flex items-center justify-between">
                    <span className="text-sm font-bold text-slate-800">📅 {sch.availableDate}</span>
                    <span className="text-xs font-bold text-green-600 bg-green-50 px-2 py-0.5 rounded-md">Available</span>
                  </div>
                  <div className="text-sm font-bold text-blue-600 mt-3 flex items-center justify-between">
                    <span>⏱️ Time: {sch.startTime?.substring(0, 5)} - {sch.endTime?.substring(0, 5)}</span>
                    <span className="text-xs font-semibold text-gray-400 group-hover:text-blue-600 transition">Select Slot ➔</span>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-md max-w-xl mx-auto space-y-6">
          ...
          <div className="space-y-3 text-sm">
            <div className="flex justify-between border-b border-gray-50 pb-2">
              ...
              <span className="font-bold text-slate-800">{selectedDoctor?.user?.name}</span>
            </div>
            <div className="flex justify-between border-b border-gray-50 pb-2">
              <span className="text-gray-400 font-medium">Specialization:</span>
              <span className="font-semibold text-blue-600">{selectedDoctor?.specialization?.name}</span>
            </div>
            <div className="flex justify-between border-b border-gray-50 pb-2">
              <span className="text-gray-400 font-medium">Channelling Date:</span>
              <span className="font-bold text-slate-800">📅 {selectedSchedule.availableDate}</span>
            </div>
            <div className="flex justify-between border-b border-gray-50 pb-2">
              <span className="text-gray-400 font-medium">Session Time:</span>
              <span className="font-bold text-slate-800">
                ⏱️ {selectedSchedule.startTime?.substring(0, 5)} - {selectedSchedule.endTime?.substring(0, 5)}
              </span>
            </div>
            <div className="flex justify-between pt-1">
              <span className="text-gray-400 font-medium">Hospital Room:</span>
              <span className="font-semibold text-slate-700">Room No 04 (Main Building)</span>
            </div>
          </div>
